...
router.get('/backbuyhistory', function (req, res) {
  buybackhistory.find({}, function (error, data) {
    res.render('backhistory', {
      data: data
    });
  });
});
router.post('/savegoods', function (req, res) {
  var pageid = Math.ceil(Math.random() * 10000000); // blog.create
...
router.post('/savegoodsfromcart', function (req, res) {
  var userid = req.body.userid; //从cart返回的有userid，bookid,cartid
  ...
  user.find({
    ...
    var cartid = '';
    if (req.body.cartid) cartid = req.body.cartid;
    bookpage.find({
      id: book_id
...
router.post('/backgoods', function (req, res) {
  buying.find({
    ...
    var buy_where = req.body.buy_where;
    var buy_money = req.body.buy_money;
    var pageid = req.body.pageid;
    var back_why = req.body.back_why;
